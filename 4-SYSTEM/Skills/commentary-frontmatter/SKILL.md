...
3. **Identify the translator, if any.** When the commentary is in a language other than the one its author used — most often a transcribed oral teaching rendered into the reader's language — record the translator under `translator`. `author` stays with the teacher. Omit `translator` when the commentary is in the author's own language.
4. **Assign a `registered_id`.** This is the short, stable identifier used in `2-RAILS/` to attribute claims to this commentary. Derive it from the author's surname in lowercase, romanised without diacritics. Once set it must never change. Check the vault annex (`4-SYSTEM/Guidelines/vault-annex.md` §Commentaries) to confirm the ID is not already taken.
5. **Identify the root text.** Record the vault path of the root text this commentary addresses under `root_text`.
6. **Determine `verse_id_format`.** This field declares the commentary's *own* internal numbering system — not the root text's. Inspect how the commentary structures its own divisions:
...
7. **Record `covers_verses`** — the range of root text verses this commentary addresses, in block-ID format (e.g., `1-1–10-58`). Omit if the range is unclear.
8. **Add external IDs** (BDRC, GRETIL, etc.) only when they are visible in the file itself.
...
translator:                   # who rendered it into this language — omit if the commentary is in the author's own language
...
author: [Commentator]
translator: [Translator — omit if not a translated commentary]
...
- **`translator` is not `author`.** For a translated commentary — including a transcribed oral teaching published in another language — `author` names the teacher and `translator` names whoever chose the words in the file. Rails built from the file quote the translator's vocabulary, so the two must never be collapsed into one field.
- **Root verses quoted inside a translated commentary usually are not the translator's own work.** Publishers commonly reprint an existing verse translation as the root text. Do not credit those verses to the commentary's translator: transclude them from the relevant `1-SOURCES/Translations/` file and add an `[Ed:...]` note recording whose rendering they are.
